...
import { createQandA } from '../../features/QandA/QandASlice';
...
const Main = () => {
    ...
    const handleSubmit = () => {
        if (!course || !week || !title || questionsArr.length <= 0) {
            setEmptySubmit(true)
            return
        }
        ...
        dispatch(createQandA(data))
            .then((res) => {
                setLoading(false);
                console.log(res)
                if (res.type === "qAndAs/createQandA/fulfilled") {
                    setCourse('')
                    setWeek('')
                    setTitle('')
                    setQuestionsArr([])
